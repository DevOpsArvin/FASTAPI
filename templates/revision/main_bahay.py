...
def get_mapping_results(station):
    return perform_search("epmap.db", f"SELECT * FROM mapping WHERE station LIKE '%{station}%'")

#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def get_vlan_results():
    return perform_search("epmap.db", f"SELECT * FROM vlans")

#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def get_voice_results():
    return perform_search("epmap.db", f"SELECT * FROM voices")

#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def get_floor_results():
    return perform_search("epmap.db", f"SELECT * FROM floors")
...
async def home(request: Request):
    ...
    print(f"**** {request.client.host} is Starting to access the NetScout Assistant - {timenow()}")
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def perform_sql2(dbase, query, data):
    conn2 = sqlite3.connect(dbase)
...
async def process_modal_form(
    ...
    data = ( port, interface, location)

    perform_sql2('epmap.db', query, data)

    okmsg = f"Updating Record - Successfull"
    ...
    resultsVLAN = get_vlan_results()
    resultsVoice = get_voice_results()
    resultsFloor = get_floor_results()
...
async def process_modal_form(
    ...
    perform_sql2('elog.db',query, data)
